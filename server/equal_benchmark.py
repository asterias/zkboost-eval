# ---------------- Resource parity (set before importing numpy!) ----------------
# Run ZKXGB_THREADS=1 python equal_benchmark.py

import os
THREADS = int(os.getenv("ZKXGB_THREADS", "1"))

# Constrain common threading backends
os.environ["OMP_NUM_THREADS"]        = str(THREADS)  # XGBoost / OpenMP
os.environ["OPENBLAS_NUM_THREADS"]   = str(THREADS)  # NumPy w/ OpenBLAS
os.environ["MKL_NUM_THREADS"]        = str(THREADS)  # NumPy w/ MKL
os.environ["VECLIB_MAXIMUM_THREADS"] = str(THREADS)  # NumPy w/ Accelerate (macOS)
...
import sys
sys.stdout = os.popen('tee -a equal_output.txt', 'w')
# ============================ Fixed-point config ============================
SCALE = 100_000
...
def run_benchmark():
    ...
    for name, loader in tqdm(dataset_loaders.items(), desc="Datasets", file=sys.stdout):
        ...
        print(f"Thread budget → XGB/OMP={THREADS}, BLAS={THREADS}")
        header = (
            f"{'depth':<6} {'trees':<6} | "
        ...
        for depth in depths:
            for trees in trees_list:
                ...
                for run in range(num_runs):
                    ...
                    # XGBoost sees float32 to align memory/bandwidth
                    ...
                    pred_fixed = models["Fixed-Wider"].predict(Xte).astype(int)
                    ...
                    # Real XGB (float32)
                    t0 = time.time()
                    models["XGB"].fit(Xtr32, ytr)
                    pred_xgb = models["XGB"].predict(Xte32)
                    t_xgb = time.time() - t0
