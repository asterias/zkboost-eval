...
class XGBoostTreeClassifierFPFast:
    ...
    def _assign_leaf_ids(self):
        """Assign deterministic leaf IDs (left-to-right pre-order). Stores self._leaf_id_map for ('leaf',val,id) tuples."""
        lid = 0
        def wrap(node):
            nonlocal lid
            if not isinstance(node, tuple):
                ...
                lid += 1
                return t
            # internal node: (feat, split_fp, l, r)
            feat, split_fp, l, r = node
            wl = wrap(l)
            wr = wrap(r)
            return (feat, split_fp, wl, wr)
        if self.tree is not None:
            self.tree = wrap(self.tree)

    def _predict_row_leaf_id(self, x_fp_row: np.ndarray, node):
        """Traverse and return leaf_id (int). Assumes leaves are ('leaf', value, id)."""
        # Support legacy ints if model loaded without IDs
        if not isinstance(node, tuple):
            return -1
        if isinstance(node[0], str) and node[0] == 'leaf':
            return int(node[2])
        feat, split_fp, l, r = node
        if x_fp_row[feat] <= split_fp:
            return self._predict_row_leaf_id(x_fp_row, l)
        else:
            return self._predict_row_leaf_id(x_fp_row, r)

    def apply_leaf_ids(self, X_fp: np.ndarray) -> np.ndarray:
        """Return matrix of leaf ids for given X (n_samples,)."""
        out = np.empty(X_fp.shape[0], dtype=np.int64)
        for i in range(X_fp.shape[0]):
...
class GenericFixedPointXGB:
    ...
    def apply_leaf_matrix(self, X: np.ndarray) -> np.ndarray:
        """Return (n_samples, n_trees) matrix of leaf IDs for the current model on X.
        Uses the same integer traversal as predict."""
        X_fp = self._X_to_fixed(X)
        n = X_fp.shape[0]
        T = len(self.trees)
                ...
                for run in range(num_runs):
                    ...
                    for label, model in models.items():
                        ...
                        if label == "Fixed-Wider":
                            # dump leaf ids (train/test) for custom model
                            try:
                                leaves_tr = model.apply_leaf_matrix(Xtr)
                                leaves_te = model.apply_leaf_matrix(Xte)
                                os.makedirs('leaves', exist_ok=True)
                                np.savetxt(f'leaves/{name}_depth{depth}_trees{trees}_train_leaves.csv', leaves_tr, delimiter=',', fmt='%d')
                                np.savetxt(f'leaves/{name}_depth{depth}_trees{trees}_test_leaves.csv',  leaves_te, delimiter=',', fmt='%d')
                            except Exception as _e:
                                print('[warn] could not save leaf ids:', _e)
                            # dump leaf ids (train/test) for custom model
                            try:
                                leaves_tr = model.apply_leaf_matrix(Xtr)
                                leaves_te = model.apply_leaf_matrix(Xte)
                                os.makedirs('leaves', exist_ok=True)
                                np.savetxt(f'leaves/{name}_depth{depth}_trees{trees}_train_leaves.csv', leaves_tr, fmt='%d', delimiter=',')
                                np.savetxt(f'leaves/{name}_depth{depth}_trees{trees}_test_leaves.csv',  leaves_te, fmt='%d', delimiter=',')
                            except Exception as e:
                                print('[warn] could not dump leaf ids:', e)
