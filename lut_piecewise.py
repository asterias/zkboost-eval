...
SIGMOID_LUT, STEP = {}, 1_000
LO, HI = -500_000, 500_000          # ±5 in fixed scale
for xf in range(LO, HI + 1, STEP):
    SIGMOID_LUT[xf] = int(round((1 / (1 + math.exp(-(xf / SCALE)))) * SCALE))
def lut_fixed_sigmoid(xf):
    xf = max(LO, min(HI, xf))
    k  = STEP * round(xf / STEP)
    return SIGMOID_LUT.get(k, SCALE // 2)
